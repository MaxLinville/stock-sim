...
def test_get_monthly_cost():
    ...
    expected_monthly_payment = 3239.98  # Calculate this value based on your formula
    monthly_payment = get_monthly_cost(house_cost, interest_rate, term_length, down_pay_amt)
    
    assert pytest.approx(monthly_payment, 0.01) == expected_monthly_payment

def test_investment():
    # Sample initial values for Investment class
    params: dict = {
        'start_cash': 15_000, # Original amount of cash from year 0
    ...
    assert investment.get_income() == params['income']
    assert investment.get_all_values() is not None
    assert investment.calculate_passive_income(0.04) >= 0

# Additional tests for other functions and methods
